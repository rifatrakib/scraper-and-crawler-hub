...
db = dataset.connect("sqlite:///quotes/quotes.db")
authors_seen = set()
...
def clean_url(url):
    ...
    # E.g. '/author/Steve-Martin' -> ['', 'author', 'Steve-Martin'] -> 'Steve-Martin'
    return path.split("/")[2]
...
def scrape_author(html_soup, author_id):
    ...
    author_born_location = html_soup.find(class_="author-born-location").get_text(strip=True)
    author_description = html_soup.find(class_="author-description").get_text(strip=True)
    
    # Store this author information
    db["authors"].insert({
        "auhtor_id": author_id,
        "name": author_name,
